...
public class GameDriveV3 extends LinearOpMode {
    ...
    private boolean scaleTheDriveInput = true;
    ...
    double scaleInput(double dVal)  {
        double[] scaleArray = { 0.0, 0.05, 0.09, 0.10, 0.12, 0.15, 0.18, 0.24,
                0.30, 0.36, 0.43, 0.50, 0.60, 0.70, 0.85, .88, 1.00 };
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive() && !isStopRequested()) {
            ...
            Vector2d input = new Vector2d(
                    -gamepad1.left_stick_y *0.82,
                    -gamepad1.left_stick_x *0.82
            ).rotated(-poseEstimate.getHeading());

            // Pass in the rotated input + right stick value for rotation
            // Rotation is not part of the rotated input thus must be passed in separately
//this section was the original unscaled power setting
if (scaleTheDriveInput) {
    drive.setWeightedDrivePower(
            new Pose2d(
                    scaleInput(input.getX()),
                    scaleInput(input.getY()),
                    -gamepad1.right_stick_x
            )
    );
}else {
//the following setting will scale the input
    drive.setWeightedDrivePower(
            new Pose2d(
                    input.getX(),
                    input.getY(),
